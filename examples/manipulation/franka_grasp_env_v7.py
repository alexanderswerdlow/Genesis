...
class FrankaGraspEnv:
    ...
    def __init__(
        ...
        # NEW: Add target position tensor and target flag: the target is a random offset from the initial cube pos.
        ...
        # self.col = CollisionManager()
        # for i in range(len(self.franka.geoms)):
        #     if 'finger' in self.franka.geoms[i].link.name:
        #         print(self.franka.geoms[i].link.name)
        #         self.col.add_object(f'franka_geom_{i}', self.franka.geoms[i].mesh._mesh)

        # assert len(self.plane.geoms) == 1
        # self.plane_mesh = self.plane.geoms[0].mesh._mesh
        # self.col.add_object(f'plane', self.plane_mesh)
    ...
    def step(self, actions):
        ...
        # self.actions[:, :-2] = torch.clamp(
        #     actions[:, :-2], -self.env_cfg["clip_actions"], self.env_cfg["clip_actions"]
        # )
        # self.actions[:, -2:] = torch.clamp(
        #     actions[:, -2:], -self.env_cfg["clip_actions_fingers"], self.env_cfg["clip_actions_fingers"]
        # )
